...
############################
  ...
  #############################################
  ##### Compiling the definitions in a PolyHok module
  #####################
  ...
  def compile_kernel(_module_name,{:defk,_,[header,[body]]}, type_def,module) do
    ...
    inf_types = PolyHok.TypeInference.type_check(delta,body)
   ...
   # raise "hell"
    ...
    cuda_body = PolyHok.CudaBackend.gen_cuda(body,inf_types,param_vars,module)
...
#    IO.puts "Compile lambda!!!!!!!!!!!!!!!!!!!!!!!!!!!!11"
 ...
 # raise "hell"
   ...
   # IO.puts "Finished infer types"
    ...
    ptr =      PolyHok.CudaBackend.gen_function_ptr(fname)
    get_ptr = PolyHok.CudaBackend.gen_get_function_ptr(fname)
